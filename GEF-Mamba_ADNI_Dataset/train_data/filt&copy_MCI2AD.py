    ...
    for filename in os.listdir(directory):
        ...
        for index, data in subset.iterrows():
            ...
            if (pd.isna(data['DXCHANGE']) == False and (
                    (int(data['DXCHANGE']) == 5 ) or (False and 1 <= int(data['DXCHANGE']) <= 3))):
        ...
        if min != 31:
            ...
            if not os.path.exists(os.path.join(Dataset,'ADNI_MRI2AD')):
                os.makedirs(os.path.join(Dataset,'ADNI_MRI2AD'))
...
directory = 'E:\PythonProject\pet2nii\dcm2nii\process02\ADNI_1'
