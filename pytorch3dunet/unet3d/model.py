...
from vit_pytorch_diy import ViT
...
class Mid_UNet(nn.Module):
    ...
    '''
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=(64, 128, 256, 512), layer_order='gcr',
                 num_groups=8, is_segmentation=True, conv_kernel_size=3, pool_kernel_size=2,
    ...
    def forward(self, x, output_mid=False):
        ...
        if output_mid:
            encoders_features.reverse(), decoders_features.reverse()
            return encoders_features, decoders_features, x
        else:
            return x

class Mid_UNet_vit(nn.Module):
    '''
    Modified from AbstractUNet, substract the code that using num_levels to directly
    set the tuple of f_maps
    '''
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=(64, 128, 256, 512), layer_order='gcr',
                 num_groups=8, is_segmentation=True, conv_kernel_size=3, pool_kernel_size=2,
                 conv_padding=1, conv_upscale=2, upsample='default', dropout_prob=0.1, is3d=True):
        super(Mid_UNet_vit, self).__init__()
        ...
        self.mid_linear = nn.Linear(960, 1024)
    ...
    def forward(self, x, output_mid=False, output_vit_mid=False):
        # encoder part
        encoders_features = []
        decoders_features = []
        for encoder in self.encoders:
        ...
        mid_output = self.mid(mid_input)
        x = rearrange(mid_output, 'b c (h md1) (md2 w) -> b c (md1 md2) h w', md1= 8, w= x.shape[-1])
...
class Residual_mid_UNet3D_vit(Mid_UNet_vit):
    """
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid=True, f_maps=(64, 128, 256, 512), layer_order='gcr',
                 num_groups=8, is_segmentation=True, conv_padding=1,
                 conv_upscale=2, upsample='default', dropout_prob=0.1, **kwargs):
        super(Residual_mid_UNet3D_vit, self).__init__(in_channels=in_channels,
                                             out_channels=out_channels,
                                             final_sigmoid=final_sigmoid,
...
class Residual_mid_UNet3D(Mid_UNet):
    """
    Residual 3DUnet model implementation based on https://arxiv.org/pdf/1706.00120.pdf.
    Uses ResNetBlock as a basic building block, summation joining instead
    of concatenation joining and transposed convolutions for upsampling (watch out for block artifacts).
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid=True, f_maps=(64, 128, 256, 512), layer_order='gcr',
                 num_groups=8, is_segmentation=True, conv_padding=1,
                 conv_upscale=2, upsample='default', dropout_prob=0.1, **kwargs):
        super(Residual_mid_UNet3D, self).__init__(in_channels=in_channels,
                                             out_channels=out_channels,
                                             final_sigmoid=final_sigmoid,
                                             basic_module=ResNetBlock,
                                             f_maps=f_maps,
...
if __name__ == "__main__":
    import torch
    device = "cuda:6"
    a = torch.randn((2, 1, 128, 128, 88)).to(device)
    model = ResidualUNet3D(in_channels=1, out_channels=1, 
                           is_segmentation=False, num_levels=4).to(device)
    model(a)
